...
for epoch in range(epochs):
    ...
    if (epoch+1) % 10 == 0:
        print(f'Epoch {epoch+1}, Loss: {"%.4f" % loss.item()}, Validation Loss: {"%.4f" % val_loss.item()}, Validation Accuracy: {"%.4f" % val_accuracy.item()}')
...
#df.to_excel('shake-LSTM.xlsx', sheet_name='sheet1', index=False)
